...
<body>
	<div class="container py-4">
		...
		<h3 class="text-center mb-4">Tour Booking: <?= $reportname ?></h3>
		...
		<table class="mb-3" border="0">
			...
			<tr>
				<td class="pr-3"><strong>Sorted by:</strong></td>
				<td>
					<?php if ($report['sort'] == 'bookingid'): ?>
						Booking ID
					<?php elseif ($report['sort'] == 'date'): ?>
						Date
					<?php elseif ($report['sort'] == 'tourtype'): ?>
						Tour Type
					<?php elseif ($report['sort'] == 'region'): ?>
						Region
					<?php endif ?>
				</td>
			</tr>
			<?php if (!isset($report['tourtype']) && !isset($report['region']) && !isset($report['status'])): ?>
				<tr>
			...
			<?php if (isset($report['tourtype'])): ?>
				<tr>
					<td class="pr-3"><strong>Tour Type: </strong></td>
					<td><?= $report['tourtype'] ?></td>
				</tr>
			<?php endif ?>
		...
		<?php else: ?>
			<table class="table table-bordered table-hover">
	          ...
	          $tourtype = $booking->tourtypename;
	          $cost = $booking->tourprice;
	        ?>
	        	<tr>
	        		<td>B_<?= $id ?></td>
	        		<td><?= $traveller ?></td>
	        		<td><?= date('d M, Y', strtotime($bookdate)) ?> <br><small>(<?= $duration ?> <?php echo ($duration==1) ? ' day' : ' days' ?>)</small></td>
	        		<td><?= $tourname ?></td>
	        		<td><?= $tourtype ?></td>
	        		<td><?= $tourguide ?></td>
